...
class SASStudioFlowOperator(BaseOperator):
    ...
    :param flow_path_type: a type that indicates what the flow_path parameter represents. valid values:
        content - the flow_path parameter represents a path in SAS Content. For example /Public/myflow.flw
        compute - the flow_path parameter represents a server file-system path that is accessible from a SAS session.
        raw - the flow_path parameter itself is a string of SAS code, or JSON representation of a flow.
    :param flow_path: path to the flow/program to execute, or actual flow/code. see above
    :param flow_exec_log: boolean. Indicates whether to dump the execution log to the Airflow log
    :param flow_type: (optional) "flow" or "program" By default this operator will execute a Studio Flow. If you specify
        "program" then it will execute a program. ie. your flow_path would either specify a path to the program
        like /Public/mycode.sas or would be the actual program itself
    :param flow_codegen_init_code: (optional) boolean. Whether to generate init code
        (default value: False)
    :param flow_codegen_wrap_code: (optional) boolean. Whether to generate wrapper code
        (default value: False)
    ...
    template_fields: Sequence[str] = ("env_vars", "macro_vars", "compute_session_id", "flow_path")
    ...
    def execute(self, context):
        if self.flow_path_type not in ['compute', 'content', 'raw']:
            raise AirflowFailException("Flow path type is invalid. Valid values are 'compute', 'content' or 'raw'")
        if self.flow_type not in ['flow', 'program']:
            raise AirflowFailException("Flow type is invalid. Valid values are 'flow' and 'program'")
    ...
    def _generate_flow_code(self):

        uri = URI_BASE

        if self.flow_path_type == "compute":
            self.log.info("Code Generation for Studio Flow stored in Compute file system")
            ...
            self.log.info("Code generation for Studio Flow stored in Content")

        media_type = "application/vnd.sas.dataflow"
        if self.flow_type == "program":
            media_type = "application/vnd.sas.program"
        req = {
            "reference": {
                "mediaType": media_type,
                "type": self.flow_path_type,
                "path": self.flow_path},
            "initCode": self.flow_codegen_initCode,
            "wrapperCode": self.flow_codegen_wrapCode,
        }
